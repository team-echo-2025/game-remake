...
export default class Player implements GameObject {
    ...
    #get_row(row: number, spritesheet?: Image): Image[] {
        if (!spritesheet) return [];
    ...
    updateDirection(): void {
        let dx = 0, dy = 0;
        if (this.pressed_keys[this.forwardKey]) dy -= 1;
        if (this.pressed_keys[this.downKey]) dy += 1;
        if (this.pressed_keys[this.leftKey]) dx -= 1;
        if (this.pressed_keys[this.rightKey]) dx += 1;
